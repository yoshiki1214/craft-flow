    ...
    for filename in os.listdir(output_dir):
        ...
        if filename.endswith(".txt") and filename.startswith("zengin_"):
            ...
            if os.path.isfile(filepath):
                ...
                history.append({"filename": filename, "datetime": dt, "formatted_datetime": formatted_dt})
...
def index():
    ...
    return render_template("bank_format/index.html", last_filename=last_filename, last_path=last_path)


@bank_format_bp.route("/history")
@login_required
@shinko_center_required
def history():
...
@bank_format_bp.route("/delete/<filename>", methods=["POST"])
@login_required
@shinko_center_required
def delete(filename):
    """変換履歴ファイルを削除"""
    try:
        output_dir = current_app.config.get(
            "OUTPUT_FOLDER", os.path.join(current_app.instance_path, "outputs")
        )
        filepath = os.path.join(output_dir, filename)
...
@bank_format_bp.route("/download/<filename>")
@login_required
@shinko_center_required
...
@bank_format_bp.route("/convert", methods=["POST"])
@login_required
@shinko_center_required
def convert():
    ...
    try:
        ...
        if not excel_path:
            ...
            if not os.path.exists(files_dir):
                flash(
                    "Excelファイルをアップロードするか、app/filesディレクトリにExcelファイルを配置してください",
                    "error",
                )
            ...
            if not excel_files:
                flash(
                    "Excelファイルをアップロードするか、app/filesディレクトリにExcelファイル（.xlsx, .xls）を配置してください",
                    "error",
                )
                return redirect(url_for("bank_format.index"))
        ...
        current_app.logger.info(
            f"変換パラメータ - シート名: {sheet_name}, 依頼人コード: {requester_code}, 依頼人名: {requester_name}"
        )
