...
class Customer(db.Model):
    ...
    updated_at = db.Column(
        db.DateTime, nullable=False, default=datetime.utcnow, onupdate=datetime.utcnow, comment="更新日時"
    )

    # テーブル制約
    __table_args__ = (
        # 銀行コードは4桁のみ許可（SQLite/PostgreSQL互換）
        CheckConstraint("LENGTH(bank_code) = 4", name="check_bank_code_length"),
        # 支店コードは3桁のみ許可
        CheckConstraint("LENGTH(branch_code) = 3", name="check_branch_code_length"),
        # 口座種別は1桁で1,2,4のみ許可
        CheckConstraint("account_type IN ('1', '2', '4')", name="check_account_type"),
        # 口座番号は7桁のみ許可
        CheckConstraint("LENGTH(account_number) = 7", name="check_account_number_length"),
        # 振込金額は0以上
        CheckConstraint("transfer_amount >= 0", name="check_transfer_amount_positive"),
        # インデックス（検索パフォーマンス向上）
...
class TransferHistory(db.Model):
    ...
    __table_args__ = (
        # 件数は0以上
        CheckConstraint("record_count >= 0", name="check_record_count_positive"),
        # インデックス（検索パフォーマンス向上）
